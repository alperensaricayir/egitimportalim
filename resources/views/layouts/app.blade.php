                    ...
                    <div class="flex items-center justify-between h-16 lg:h-18 gap-4">
                        <div class="flex items-center gap-4 lg:gap-6">
                            <div class="shrink-0 flex items-center">
                                <a href="{{ route('courses.index') }}" class="flex items-center gap-3">
                                    <img src="{{ asset('images/3sgrupENiyilogo.png') }}" alt="3S Grup" class="h-10 w-auto object-contain" />
                                </a>
                            </div>
                            <details class="relative hidden lg:block">
                                ...
                                <div class="absolute left-0 mt-2 w-64 rounded-xl border border-gray-200 bg-white shadow-lg ring-1 ring-black/5 dark:border-gray-700 dark:bg-gray-900">
                                    <div class="p-2 grid grid-cols-1 gap-1 text-sm">
                                        <a href="{{ route('courses.index') }}" class="flex items-center justify-between rounded-lg px-3 py-2 text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800">
                                            <span>{{ __('ui.courses') }}</span>
                                            <span class="text-xs text-gray-500">›</span>
                                        </a>
                                        @if (Route::has('products.index'))
                                            <a href="{{ route('products.index') }}" class="flex items-center justify-between rounded-lg px-3 py-2 text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800">
                                                <span>{{ __('ui.products') }}</span>
                                                <span class="text-xs text-gray-500">›</span>
                                            </a>
                                        @endif
                                        @if (Route::has('jobs.index'))
                                            <a href="{{ route('jobs.index') }}" class="flex items-center justify-between rounded-lg px-3 py-2 text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800">
                                                <span>{{ __('ui.jobs') }}</span>
                                                <span class="text-xs text-gray-500">›</span>
                                            </a>
                                        @endif
                                        @if (Route::has('public.profiles.index'))
                                            <a href="{{ route('public.profiles.index') }}" class="flex items-center justify-between rounded-lg px-3 py-2 text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800">
                                                <span>{{ __('ui.members') ?? 'Üyeler' }}</span>
                                                <span class="text-xs text-gray-500">›</span>
                                            </a>
                                        @endif
                                        @if (Route::has('support.index'))
                                            <a href="{{ route('support.index') }}" class="flex items-center justify-between rounded-lg px-3 py-2 text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800">
                        ...
                        <div class="flex items-center gap-3 lg:gap-4">
                            <div class="flex items-center gap-4 text-sm font-medium text-gray-700 md:hidden dark:text-gray-300">
                                <a href="{{ route('courses.index') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                                    {{ __('ui.courses') }}
                                </a>
                                @auth
                            ...
                            <div class="hidden sm:flex items-center gap-2 lg:gap-3">
                                <div class="flex items-center gap-1">
                                    <form method="POST" action="{{ route('locale.update') }}">
                                        ...
                                        <button type="submit" class="rounded-md px-2 py-1 text-sm {{ app()->getLocale()==='tr' ? 'ring-2 ring-indigo-500 font-semibold' : '' }} hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Türkçe">🇹🇷</button>
                                    </form>
                                    <form method="POST" action="{{ route('locale.update') }}">
                                        @csrf
                                        <input type="hidden" name="locale" value="en">
                                        <button type="submit" class="rounded-md px-2 py-1 text-sm {{ app()->getLocale()==='en' ? 'ring-2 ring-indigo-500 font-semibold' : '' }} hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="English">🇬🇧</button>
                                    </form>
                                </div>
                                <button
                                    id="theme-toggle"
                                ...
                                @auth
                                    <div x-data="{ open: false }" class="relative">
                                        <button @click="open = !open" class="flex items-center gap-2 rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm font-medium text-gray-800 hover:bg-gray-100 hover:border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-700">
                                            <img class="h-6 w-6 rounded-full object-cover" src="{{ Auth::user()->profile?->avatar ? asset('storage/' . Auth::user()->profile->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&color=7F9CF5&background=EBF4FF' }}" alt="{{ Auth::user()->name }}" />
                                        ...
                                        <div
                                            ...
                                            <a href="{{ route('my.profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0">Profilim</a>
                                            @if (Route::has('settings.index'))
                                                <a href="{{ route('settings.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-1">Ayarlar</a>
                                            @endif
                                ...
                                @else
                                    @if (Route::has('login'))
                                        <a href="{{ route('login') }}" class="text-sm font-medium text-gray-700 hover:text-indigo-600 dark:text-gray-200 dark:hover:text-indigo-400">
                                            {{ __('ui.login') }}
                                        </a>
                                    @endif
                                    @if (Route::has('register'))
                                        <a href="{{ route('register') }}" class="rounded-full bg-indigo-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-indigo-700">
                                            {{ __('ui.register') }}
                                        </a>
                                    @endif
                                @endauth
