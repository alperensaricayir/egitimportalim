                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($product->discounted_price)
                                                ...
                                                <span class="text-green-600 font-bold ml-1">{{ $product->discounted_price }}</span>
                                            @else
                                                {{ $product->price }}
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('admin.products.edit', $product) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('ui.edit') }}</a>
                                            <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('ui.delete') }}?');">
                                                @csrf
                                                @method('DELETE')
